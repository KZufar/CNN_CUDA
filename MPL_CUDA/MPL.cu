...
__global__ void _input_fc1(int j)
{
	...
	data[tid] = _train_image[j][ix][iy] * _fc1_w[ib][ix][iy];
	//data[tid] = 1;
	__syncthreads();
...
__global__ void _fc1_fc2()
{
	...
	int tid = threadIdx.x;
	data[tid] = _fc1_a[ix]* _fc2_w[ib][ix];
	//data[tid] = 1;
	__syncthreads();
...
int main() {
	...
	for (int i = 1;i <= epochs;i++)
	{
		...
		for (int j = 1;j < TRAIN_NUM; j++)
		{
			...
			_input_fc1 << <FC1_SIZE, dim3(ROW, COL) >> > (j);
			_fc1_fc2 << <FC2_SIZE, FC1_SIZE>> > ();
			//_input_fc1 << <dim3(TRAIN_NUM, FC1_SIZE, dim3(ROW, COL) >> > ();
